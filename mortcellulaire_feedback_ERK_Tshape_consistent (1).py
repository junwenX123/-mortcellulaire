...
# ============================================================
# Gillespie / event-driven simulation for the model in the PDF,
# with one explicit extension: a fixed T-shaped background zone.
#
...
# The PDF takes the theoretical horizon T = infinity; in simulation we stop at T_max.
# To stop after 200 observed cell deaths, set target_deaths = 200 and T_max = np.inf.
T_max = 20.0
target_deaths = None
...
def local_activation_intensity(x, y):
    """
    Return the T-shaped extension of the PDF activation intensity.

    PDF without T-shape:
        lambda_a_1 inside A(V^a_t), lambda_a_c outside.

    Here, to keep your T-shaped zone:
