...
def death_statistics(result: SimulationResult) -> Dict[str, float | int | str | bool]:
    ...
    return {
        ...
        "death_rejection_rate": result.n_death_rejected / result.n_death_candidates if result.n_death_candidates > 0 else np.nan,
        "death_acceptance_rate_total": result.n_observed_deaths_total / result.n_death_candidates if result.n_death_candidates > 0 else np.nan,
...
def make_scenarios(base: Parameters) -> Dict[str, Parameters]:
    """
    Parameter sweep focused on reducing death-candidate rejection.

    In the baseline, many death candidates are rejected because they fall outside
    active zones or inside ERK protection zones. The scenarios below reduce this
    rejection by making ERK protection weaker/shorter or by making active regions
    more available.
    """
    return {
        "baseline": base,

        # Optimization 1: shorter ERK lifetime.
        # Mean ERK lifetime = 1 / beta_d_T, so increasing beta_d_T makes protection disappear faster.
        "shorter_ERK_duration": replace(base, seed=201, beta_d_T=2.0),

        # Optimization 2: smaller ERK radius.
        # Mean ERK radius = 1 / beta_d_R, so increasing beta_d_R makes protected disks smaller.
        "smaller_ERK_radius": replace(base, seed=202, beta_d_R=3.0),

        # Optimization 3: mild combined change.
        # Slightly smaller/shorter ERK protection and slightly more persistent/larger active zones.
        "optimized_mild": replace(
            base,
            seed=205,
            beta_d_T=1.5,
            beta_d_R=2.5,
        ...
        # Optimization 4: stronger combined change
        "optimized_strong": replace(
            base,
            seed=206,
            beta_d_T=2.0,
            beta_d_R=3.0,
        ...
        # Optional T-zone comparison that keeps rejection reasonable.
        "moderate_T_zone_activation": replace(
            base,
            seed=207,
            lambda_a_T=1.0,
            lambda_a_c=0.10,
...
def main() -> None:
    out_dir = Path("simulation_outputs_1000_unified_burnin_optimized_rejection")
    out_dir.mkdir(parents=True, exist_ok=True)

    base = Parameters(burn_in_deaths=500, target_deaths=1000)
    scenarios = make_scenarios(base)

    print("Running optimized unified Gillespie simulations: burn-in 500 deaths, then analyze 1000 deaths...\n")
    ...
    for result in results:
        ...
        print(f"  death acceptance rate:  {stats['death_acceptance_rate_total']:.4f}")
